...
@section('content')
    ...
    <main class="px-3 md:mx-14 md:mb-40">
            ...
            <x-carousel.slider-category>
                @foreach ($Carousel as $item)
                    <swiper-slide
                        ...
                        <a href="/kategori/{{ $item['Title'] }}" class="flex flex-col justify-evenly h-full items-center">
        ...
        <x-list.slider>
            @forelse ($RekomendasiWarung as $item)
                <swiper-slide
                    class="w-96 h-[17rem] mx-0.5 relative border-2 rounded-xl transition-all duration-300 hover:shadow-md">
                    ...
                    <a href="/detail-warung/{{ $item->id }}" class="w-full h-full bg-white overflow-hidden">
                        <img src="{{ Storage::url($item->logo_umkm) }}" alt=""
                            class="w-[45rem] h-40 object-cover rounded-xl">
        ...
        <x-list.slider-makanan>
            @foreach ($RekomendasiMakanan as $menu)
                ...
                <swiper-slide
                    class="h-[20rem] w-96 flex flex-col relative justify-between bg-white border-2 rounded-xl overflow-hidden transition-all duration-300 my-2 hover:shadow-md">
                    <img src="{{ Storage::url($menu->image) }}" alt=""
                        class="w-full h-[14rem] object-cover relative">
                    {{-- Description Card --}}
            ...
            <x-list-food.slider>
                @foreach ($RekomendasiMakanan as $menu)
                    @php $harga = number_format($menu->harga, 0, ',', '.'); @endphp
                    <div
                        class="food-list-scrollTrigger w-full h-max flex flex-col relative justify-evenly bg-white border-2 rounded-xl overflow-hidden transition-all duration-300 my-2 hover:shadow-md">
                        <img src="{{ Storage::url($menu->image) }}" alt=""
                            class="w-[40rem] h-60 object-cover relative top-0">
                        {{-- Description Card --}}
                        <div class="w-full flex flex-row items-center justify-between px-3 py-4">
                            {{-- Title & Warung --}}
    ...
    <footer
        class="md:grid hidden grid-cols-4 w-full bg-gradient-to-t from-[#ED6600] to-[#F9832A] text-white h-[40vh] place-content-evenly px-10 place-items-stretch">
