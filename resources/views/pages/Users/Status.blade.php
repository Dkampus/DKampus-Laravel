...
<div class="flex flex-col gap-5 items-center justify-start bg-[#F0F3F8]">
    ...
    <section id="card" class="bg-white w-full flex flex-col shadow-md justify-center items-center">
...
<script>
    ...
    database.ref('needToDeliver').on('child_added', function(snapshot) {
        ...
        database.ref('onProgress').on('child_added', function(snapProgress) {
            var idProggress = snapProgress.split('-')[0];
            if (idProggress == custId) {
        ...
        if (id == custId) {
            ...
            display(nama_umkm, tanggal, status, items, orderId, formattedTotal);
        };
    });

    function display(umkm, tanggal, status, items, orderId, jumlah) {
        ...
        var divStatus = $('<div>').addClass('bg-[#FFEEE1] rounded-2xl text-center flex flex-col justify-center text-[#FF6E00] text-sm font-semibold py-2.5 px-3').text(status).attr('data-courId', );
    ...
    <svg height="30" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        var a_lacak = $('<a>').addClass('bg-[#F9832A] w-20 h-9 flex flex-col items-center justify-center rounded-xl text-white').text('Lacak');
        var a_chat = $('<a>').addClass('w-6');
        var imgChat = $('<img>').addClass('w-full').attr('src', '/bubbleChat.svg').attr('alt', '');
        ...
        $('#card').append(divContent, divFooter);
    }
</script>