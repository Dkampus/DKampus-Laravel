...
@section('pesananContent')
    @php
        //example data for dummy
            $carts = [
                [
                    'store_name' => 'Warung Ayam Baghdad',
                    'date' => '2022-01-01',
                    'status' => 'Diproses',
                    'items' => '3 Ayam Bakar Madura, 2 Ayam Madu, 4 Nasi Putih',
                    'id' => 'trxxxxxxxxxxxx',
                    'total_price' => '50000',
                ],
                [
   ...
   <div class="flex flex-col gap-5 items-center justify-start bg-[#F0F3F8]">
       @foreach ($carts as $cart)
           <section id="{{ str_replace(' ', '-', strtolower($cart['store_name'])) }}" class="bg-white w-full flex flex-col shadow-md justify-center items-center">
               <div id="contentStatus" class="w-[29.5rem] relative h-auto overflow-hidden border-b-[3px] px-4 my-3 mx-auto flex flex-col items-center py-5">
                   <div id="Title" class="border-b-[3px] pb-5 mx-auto flex flex-row justify-between items-center w-full">
                       <div id="title" class="flex flex-row items-center gap-5">
                           ...
                           <div>
                               <h1 class="font-semibold text-lg">{{ $cart['store_name'] }}</h1>
                               <p id="date">{{ $cart['date'] }}</p>
                           </div>
                       </div>
                   ...
                   <div id="Content" class="w-full grid grid-cols-1 grid-rows-2 gap-y-1 gap-x-5 my-5 place-content-between">
                       ...
                       <p id="orderid" class="">Order ID
                           <span class ="font-semibold flex">#{{ $cart['id'] }}</span>
                       </p>
                   </div>
               </div>
               ...
               <div class="flex flex-row justify-between w-[29.5rem] px-5 py-1">
                   ...
                   <div id="LacakAndChat" class="flex flex-row items-center gap-5">
                       <a href="{{ route('status.order', [$cart['id']]) }}" class="bg-[#F9832A] w-20 h-9 flex flex-col items-center justify-center rounded-xl text-white">Lacak</a>
