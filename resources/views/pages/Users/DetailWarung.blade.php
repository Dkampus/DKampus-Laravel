...
<header class="relative">
    ...
    {{-- Navbar --}}
    <nav id="navbar-detail-warung" class="fixed transition-all duration-200 z-[60] py-3.5 w-full flex flex-row justify-between items-center px-6 mt-10 mb-5">
        ...
        <label for="NavCheckbox" id="NavLikeButton" class="w-12 h-12 bg-white flex flex-col justify-center items-center rounded-full">
            <svg id="NavLikeIcon" height="25" class="fill-[#5e5e5e]/80 transition-all duration-300" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <input type="checkbox" id="NavCheckbox" class="form-checkbox opacity-0 absolute invisible" />

    </nav>

    <div class="h-[23rem] md:h-auto md:max-h-[25rem] max-w-full md:h-full md:w-full">
        <img src="{{ Storage::url($umkm_img) }}" alt="" class="object-cover h-full md:h-auto md:max-h-[25rem] w-full">
    ...
    <div id="desc" class="absolute text-white bottom-0 z-50 mx-5 my-5">
        ...
        {{-- Judul Warung --}}
        <h1 class="font-bold text-4xl mb-2 md:text-5xl">{{$nama_umkm}}</h1>

        {{-- Judul Tempat --}}
        <h2 class="mb-5 text-lg md:text-xl">{{$alamat}}</h2>

        {{-- Ratings & Clock --}}
        <div id="ratings&clock" class="flex flex-row gap-5">
            {{-- Ratings --}}
            <div id="Ratings" class="flex flex-row items-center text-lg gap-2 md:text-xl">
                <div id="averageRatings" class="flex flex-row items-center gap-1">
            ...
            <div id="time" class="flex flex-row items-center text-lg gap-1 md:text-xl">
                ...
                <h1>@php echo date('H:i', strtotime($open_time)); @endphp - @php echo date('H:i', strtotime($close_time)); @endphp</h1>
            </div>
        </div>
    </div>
</header>
            ...
            <div class="w-full sm:w-[15rem] relative mb-2 border rounded-2xl transition-all duration-300 overflow-hidden hover:shadow-md">
                ...
                <div id="desc" class="flex flex-col gap-2 py-2 px-3">
                    ...
                    <div id="buttons" class="flex flex-row gap-2 items-center">
                        ...
                        <input type="checkbox" id="CardCheckbox" class="form-checkbox opacity-0 absolute invisible" />
                    </div>
                </div>
            </div>
        @empty
