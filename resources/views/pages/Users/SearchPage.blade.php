        ...
        <div class="grid grid-cols-2 gap-5 p-2">
            @foreach($menus as $menu)
                <div class="w-full h-full bg-white rounded-xl shadow-md overflow-hidden m-3">
                    ...
                    <div class="p-2">
                        <div class="uppercase tracking-wide text-sm text-indigo-500 font-semibold">{{ $menu->nama_makanan }}</div>
                        <div class="flex items-center mt-2">
                            <img src="{{ asset('shop.svg') }}" alt="" class="w-5 h-5">
                            <p class="text-[#5E5E5E] text-m ml-2"> Nama umkm </p>
                        </div>
                        <p class="block mt-1 text-lg leading-tight font-medium text-black">Rp {{ number_format($menu->harga, 0, ',', '.') }}</p>
                        <div class="flex items-center mt-2">
