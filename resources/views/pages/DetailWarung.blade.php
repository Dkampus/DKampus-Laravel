...
<header class="relative">
    ...
    <nav id="navbar-detail-warung" class="fixed transition-all duration-200 z-[60] py-3.5 w-full flex flex-row justify-between items-center px-6 mt-10 mb-5">
        ...
        {{-- Title --}}
        <h1 id="titleDetail" class="text-white font-semibold uppercase invisible">Baghdad Fried Chicken</h1>

        {{-- Favorit --}}
        <button id="buttonLike" class="w-12 h-12 bg-white flex flex-col justify-center items-center rounded-full">
            <svg id="iconLike" height="25" class="fill-[#5e5e5e] transition-all duration-300" viewBox="0 0 19 18" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<main>
    ...
    <div class="grid grid-cols-2 relative gap-x-10 gap-y-5 mx-6 place-content-center place-items-center">
        {{-- Discount Img --}}
        <img src="discount50%.svg" alt="" class="top-5 -left-[1.34rem] z-50 absolute w-20">

        {{-- Card Makanan --}}
        @forelse ($CardFood as $Item)
            <a href="/detail-makanan" class="w-[15rem] relative mb-2 border rounded-2xl transition-all duration-300 overflow-hidden hover:shadow-md">
